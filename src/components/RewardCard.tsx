...
const RewardCard: React.FC<Props> = ({ reward }) => {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      setMessage(res?.message ?? (res?.success ? 'Redeemed successfully!' : 'Could not redeem reward.'))
    } catch (err: any) {
    ...
    <article className="bg-slate-800/60 border border-slate-700 rounded-xl p-4 flex flex-col shadow-md hover:shadow-lg transition">
      <div className="relative rounded-md overflow-hidden h-40 mb-4 bg-linear-to-br from-green-500/30 to-emerald-400/10 flex items-center justify-center">
        ...
        {reward.image ? (
          ...
          <div className="text-white text-lg font-semibold opacity-90">{reward.title}</div>
        )}

        {/* ribbon */}
        {reward.popular && (
          <span className="absolute left-3 top-3 bg-amber-400 text-slate-900 text-xs font-semibold px-2 py-1 rounded">Popular</span>
        )}
      </div>
      ...
      <p className="text-slate-300 text-sm flex-1 my-3 leading-relaxed">{reward.description}</p>

      <div className="flex items-center justify-between gap-3">
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-bold text-emerald-300">{reward.points}</span>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={`Redeem ${reward.title}`}>
        <div className="text-slate-300 mb-4">
          Are you sure you want to redeem <strong className="text-white">{reward.title}</strong> for <strong className="text-emerald-300">{reward.points} pts</strong>?
        </div>

        <div className="flex justify-end gap-3">
          <button onClick={() => setOpen(false)} className="px-4 py-2 rounded bg-slate-700 text-slate-200">Cancel</button>
          <button onClick={onConfirm} disabled={redeeming} className="px-4 py-2 rounded bg-emerald-500 text-white font-semibold disabled:opacity-60">
