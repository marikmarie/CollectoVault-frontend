...
export default function VendorPointRulesPage(): JSX.Element {
  ...
  function mapFromApi(r: any): PointRule {
    return {
      ...
      maxPerTransaction: r.max_per_transaction == null ? null : Number(r.max_per_transaction),
  ...
  const fetchRules = async () => {
    ...
    try {
      ...
      setRules((data || []).map(mapFromApi));
    } catch (err) {
      console.error("Failed to load point rules", err);
      setToast({ msg: "Failed to load rules.", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (!sessionLoading && vendorId) fetchRules();
  ...
  const saveRule = async (payload: Partial<PointRule>) => {
    if (!vendorId) return setToast({ msg: "Vendor not available", type: "error" });
    if (!payload.name?.trim()) return setToast({ msg: "Rule name is required", type: "error" });
    if (!payload.trigger) return setToast({ msg: "Trigger is required", type: "error" });
    ...
    try {
      ...
      const body = {
        ...
        max_per_transaction: payload.maxPerTransaction ?? null,
        max_per_day: payload.maxPerDay ?? null,
      ...
      if (editing) {
        await api.put(`/api/vendor/${vendorId}/point-rules/${editing.id}`, body);
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="grid gap-4">
        {loading ? (
          ...
          <Card key={r.id} className="p-4 flex flex-col sm:flex-row justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                <div className={`px-2 py-1 rounded text-sm font-medium ${r.active ? "bg-emerald-600/20 text-emerald-300" : "bg-slate-800 text-slate-400"}`}>
                  {(r.trigger || "").toUpperCase()}
                </div>
                <div>
...
function RuleForm({ initial, onSave, onCancel, saving }: any) {
  const [name, setName] = useState(initial?.name ?? "");
  const [desc, setDesc] = useState(initial?.description ?? "");
  const [trigger, setTrigger] = useState<TriggerType>(initial?.trigger ?? "purchase");
  const [points, setPoints] = useState(initial?.points ?? 0);
  const [multiplier, setMultiplier] = useState(initial?.multiplier ?? undefined);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <input className="w-full px-3 py-2 rounded bg-slate-900/50 border border-slate-700"
          value={name} onChange={e => setName(e.target.value)} />
      </div>

      <div>
        <label className="block text-sm">Description (optional)</label>
        <textarea className="w-full px-3 py-2 rounded bg-slate-900/50 border border-slate-700"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          ...
          <select className="w-full px-3 py-2 rounded bg-slate-900/50 border border-slate-700"
        ...
        <div>
          ...
          <input type="number" min={0} className="w-full px-3 py-2 rounded bg-slate-900/50 border border-slate-700"
            value={points} onChange={e => setPoints(Number(e.target.value))} />
        </div>

        <div>
          <label className="block text-sm">Multiplier (optional)</label>
          <input type="number" min={0} step="0.1" className="w-full px-3 py-2 rounded bg-slate-900/50 border border-slate-700"
            value={multiplier ?? ""} onChange={e => setMultiplier(e.target.value === "" ? undefined : Number(e.target.value))} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <input type="number" min={0} className="w-full px-3 py-2 rounded bg-slate-900/50 border border-slate-700"
            value={maxPerDay} onChange={e => setMaxPerDay(e.target.value === "" ? "" : Number(e.target.value))} />
        </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button onClick={() => onSave({
          ...
          points,
          multiplier,
          maxPerDay: maxPerDay === "" ? null : maxPerDay,
          active
