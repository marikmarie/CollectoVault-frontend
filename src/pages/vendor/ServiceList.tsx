            ...
            setServices([
              {
                id: "s1",
                title: "Spa voucher",
                description: "Relaxing 2-hour spa",
                pricePoints: 1200,
                priceCurrency: 15,
                active: true,
  ...
  const toggleActive = async (id: string, next: boolean) => {
    ...
    try {
      if ((vendorService as any)?.updateService) {
        ...
        setServices((s) =>
          s.map((x) => (x.id === id ? { ...x, active: next } : x))
        );
      } else {
        setServices((s) =>
          s.map((x) => (x.id === id ? { ...x, active: next } : x))
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-300">
            Manage, edit, or remove services offered to customers.
          </p>
        </div>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <div className="p-6 text-center text-slate-400">
            No services yet. Upload one to begin accepting redemptions.
          </div>
        ) : (
                ...
                <tr key={s.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="text-sm">
                      {s.pricePoints ? `${s.pricePoints} pts` : "—"}
                    </div>
                    <div className="text-xs text-slate-400">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-1 rounded text-sm font-medium ${
                        s.active ? "bg-emerald-600" : "bg-slate-700"
                      }`}
                    >
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex items-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          navigate(`/vendor/services/${s.id}/edit`)
                        }
                        className="px-3 py-1 text-sm rounded border border-slate-700 hover:bg-slate-800"
                      >
                        Edit
                      ...
                      <button
                        onClick={() => remove(s.id)}
                        disabled={busyId === s.id}
                        className="px-3 py-1 text-sm rounded bg-rose-500 hover:bg-rose-600 text-white"
                      >
                        Delete
                      </button>
                    </div>
